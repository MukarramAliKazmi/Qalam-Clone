...
async function removeTeachers(req, res) {
    ...
    await con.query(`delete from student_courses_temp where teacherid = ${req.body.teacherid}`, function (err, teacher, fields) {
        
        if (err)    console.log("Error:3")
        else        console.log(`removed ${req.body.teacherid}`)
    })

    await con.query(`delete from student_courses_previous where teacherid = ${req.body.teacherid}`, function (err, teacher, fields) {
